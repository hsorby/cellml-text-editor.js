export class CellMLLatexGenerator {
  ...
  private parseApply(node: Element | null | undefined): string {
    ...
    switch (op) {
      case "plus": return args.join(" + ");
      case "minus": return args.length === 1 ? `-${args[0]}` : `${args[0]} - ${args[1]}`;
      case "times": return args.join(" \\cdot ");
      case "divide": return `\\frac{${args[0]}}{${args[1]}}`;
      case "eq": return `${args[0]} = ${args[1]}`;
      case "power": return `{${args[0]}}^{${args[1]}}`;
      case "root": return `\\sqrt{${args[0]}}`; // simple sqrt
      case "diff": 
      ...
      // Trig & Funcs
      case "sin": case "cos": case "tan": case "exp": case "log": case "ln":
        return `\\${op}\\left(${args[0]}\\right)`;
        
      default: return `\\text{${op}}(${args.join(", ")})`;
    }
  }
